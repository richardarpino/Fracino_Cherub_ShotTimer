...
public:
    ...
    void update() override {
        ...
        Reading parent = registry->getLatest<BoilerPressureReading>();
        registry->publish<HeatingCycleReading>(Reading(parent.value + 1.0f, "", "DERIVED", 0, false, PhysicalQuantity::COUNTER));
    }
};
...
void test_auto_propagation_missing() {
    ...
    dispatcher.attachProcessor<HeatingCycleReading>(&processor);

    // Publish parent
    dispatcher.publish<BoilerPressureReading>(Reading(1.0f, "BAR", "", 0, false, PhysicalQuantity::PRESSURE));
    ...
    Reading child = dispatcher.getLatest<HeatingCycleReading>();
